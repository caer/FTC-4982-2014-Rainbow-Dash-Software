...
task main()
{
	...
	//Disable joystick auto updating.
	...
	//Start the modules.
	StartTask(dragonflyListener);
	StartTask(crossbowListener);
	StartTask(autonomousListener);

	//Loop until the autonomous phase ends.
	...
	//Stop the Autonomous listener.
	StopTask(autonomousListener);

	//Wait for the tele-op phase to begin.
	waitForStart();
